...
void userMenu() { // mi logica de programacion termina con la implementacion de esta funcion, me esta dando dolores de cabeza hacer las llamadas dentro de esta funcion y no lo consigo
	cout << "\n" << endl;
	...
	switch (seleccion) {
	case 1:
		cout << "Panel no trabajado, cerrando programa. Realice los trabajos como administrador porfavor y gracias" << endl;
		break;
	case 2:
		cout << "Panel no trabajado, cerrando programa. Realice los trabajos como administrador porfavor y gracias" << endl;
		break;
...
void gestionPanel() {
	...
	char letra = 'x'; // me obliga a inicializar esto para que funcione con una letra interior. Lo tengo que inicializar fuera porque le da la gana al programa.
	...
	case 2:
		...
		if (modifyPass == 0) { // Avisaremos en caso que se intente cambiar el password de administrador
			...
			cout << "Introduce de nuevo la contrasenia" << endl;
			string confirmacion;
			cin >> confirmacion;

				if (confirmacion != newpass) {
					cout << "Error, Verificacion de contrasenia incorrecta" << endl;
					}
					else {
	...
	case 3:
		...
		cout << "Estas seguro que quieres realizar el borrado del usuario especificado? (S/N)" << endl;
...
void adminMenu() {
	...
	cout << "5. salir (cerrar programa)" << endl; // eso entiendo segun el enunciado
